...
export default function CreateUserModal({ open, onClose, onCreated }) {
  ...
  const [form, setForm] = useState({ username: '', displayName: '', password: '', confirm: '', role: 'user' })
  ...
  async function handleSubmit(e) {
    e.preventDefault()
    setError('')
    if (!USERNAME_RE.test(form.username)) { setError('Username must be 3-20 chars: letters, numbers, underscores only.'); return }
    ...
    setForm({ username: '', displayName: '', password: '', confirm: '', role: 'user' })
    onCreated?.()
    onClose()
  }
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input value={form.username} onChange={e => update('username', e.target.value.toLowerCase().replace(/[^a-z0-9_]/g, ''))}
              placeholder="john_doe" maxLength={20} className="input-dark" />
            <p className="text-xs text-slate-600 mt-1">3-20 chars, lowercase, _ allowed</p>
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1.5">Display Name</label>
            <input value={form.displayName} onChange={e => update('displayName', e.target.value)} placeholder="John Doe" className="input-dark" />
          </div>
        </div>
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1.5">Password</label>
            <input type="password" value={form.password} onChange={e => update('password', e.target.value)} placeholder="Min 8 chars" className="input-dark" />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1.5">Confirm Password</label>
            <input type="password" value={form.confirm} onChange={e => update('confirm', e.target.value)} placeholder="••••••••" className="input-dark" />
          </div>
        </div>
        <div>
